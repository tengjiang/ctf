...
/*
This is my modified version of heapoflove.c for testing purposes.
*/
...
void display_gender(user_account* user) {
	switch(user->gender) {
		...
		case 1337:
            ...
            // printf("You have %d fingers, which is the perfect number for typing on a keyboard.\n", user->fingers);
            // printf("Your name: %s\n", user->name);
            // printf("Your gender: %d\n", user->gender);
...
int main(void) {
    ...
    // printf("malloc_usable_size(user_name): %d\n", malloc_usable_size(user->name));
    // printf("User address: %p\n", user);
    // printf("User name address: %p\n", user->name);
    // printf("Sizeof pointer: %d\n", sizeof(user->name));
    // printf("offset: %lu\n", (unsigned long)user - (unsigned long)(user->name) );
